...
pub(crate) struct GradingArgs {
    ...
    #[arg(long, env = "DS_GRADING_UPLOAD_LISTEN", default_value = "10.20.0.1:7202")]
    pub grading_upload_listen: SocketAddr,
    #[arg(long, env = "DS_GRADING_RESULTS_LISTEN", default_value = "10.20.0.1:7201")]
...
fn protected(path: &Path, directory: bool) -> anyhow::Result<()> {
    let meta = std::fs::symlink_metadata(path)?;
        ...
        "grading state must be private, owned, and not a symlink: {}", path.display()
    );
    if !directory {
...
fn load_private<T: serde::de::DeserializeOwned>(path: &Path) -> anyhow::Result<T> {
    ...
    ensure!(file.metadata()?.len() <= MAX_LEDGER_BYTES, "grading state too large");
    let mut bytes = Vec::new();
    file.take(MAX_LEDGER_BYTES + 1).read_to_end(&mut bytes)?;
    ensure!(bytes.len() as u64 <= MAX_LEDGER_BYTES, "grading state too large");
...
fn persist_private<T: Serialize>(path: &Path, value: &T) -> anyhow::Result<()> {
    ...
    ensure!(bytes.len() as u64 <= MAX_LEDGER_BYTES, "grading ledger quota");
...
fn validate_scorer_keys(keys: &BTreeMap<String, String>) -> anyhow::Result<()> {
    ensure!(!keys.is_empty() && keys.len() <= 64, "1..64 scorer key versions required");
    for (version, key) in keys {
        ensure!(valid_id(version) && valid_digest(key), "invalid scorer key version");
        pubkey_from_hex(key)?;
    }
    Ok(())
}
...
impl GradingService {
    fn open(
        ...
        if initialize {
            ensure!(!anchor_path.exists() && !dir.exists(), "grading initialization is one-time only");
            let max_jobs = budget.context("--grading-init requires --grading-budget")?;
            ensure!((1..=MAX_BUDGET).contains(&max_jobs), "grading budget must be finite (1..4096)");
            let anchor = Anchor { v: FORMAT, ledger_id: random_hex(32), max_jobs };
            ...
            std::fs::DirBuilder::new().mode(0o700).create(dir.join("objects"))?;
            create_private(&dir.join("broker.json"), &BrokerLedger {
                v: FORMAT, ledger_id: anchor.ledger_id.clone(), wall_high_water: 0,
                minute: 0, minute_attempts: 0, authorizations: BTreeMap::new(),
                submissions: BTreeMap::new(),
            })?;
            create_private(&dir.join("ledger.json"), &GradingLedger {
                v: FORMAT, ledger_id: anchor.ledger_id.clone(), max_jobs,
                spent: 0, high_water: 0, wall_high_water: 0, scorer_keys: keys.clone(),
                claims: BTreeMap::new(),
            })?;
        } else {
            ensure!(budget.is_none(), "budget is immutable; supply it only for initialization");
        }
        let anchor: Anchor = load_private(&anchor_path).context("grading anchor missing/corrupt; refusing reset")?;
        ensure!(anchor.v == FORMAT && valid_digest(&anchor.ledger_id)
            && (1..=MAX_BUDGET).contains(&anchor.max_jobs), "invalid grading anchor");
        protected(&dir, true)?;
        ...
        for (version, pinned) in &ledger.scorer_keys {
            ensure!(keys.get(version) == Some(pinned), "scorer key removal/replacement forbidden");
        }
    ...
    fn validate_state(anchor: &Anchor, broker: &BrokerLedger, ledger: &GradingLedger) -> anyhow::Result<()> {
        ensure!(broker.v == FORMAT && ledger.v == FORMAT && broker.ledger_id == anchor.ledger_id
            && ledger.ledger_id == anchor.ledger_id && ledger.max_jobs == anchor.max_jobs,
            "grading ledger/anchor mismatch");
        ensure!(broker.authorizations.len() <= MAX_UPLOAD_AUTHORIZATIONS
            && broker.submissions.len() <= MAX_SUBMISSIONS
            && broker.minute_attempts <= MAX_UPLOADS_PER_MINUTE, "broker quota corruption");
        validate_scorer_keys(&ledger.scorer_keys)?;
        ensure!(ledger.spent <= ledger.max_jobs && ledger.spent == ledger.claims.len() as u64
            && ledger.high_water == ledger.spent, "grading budget/high-water corruption");
        for (run, auth) in &broker.authorizations {
            ensure!(valid_id(run) && valid_id(&auth.incarnation) && valid_digest(&auth.token_digest)
                && auth.attempts <= MAX_UPLOAD_ATTEMPTS, "invalid upload authorization");
            if let Some(digest) = &auth.submission_digest {
                let sub = broker.submissions.get(digest).context("missing submission claim")?;
                ensure!(sub.run_id == *run && sub.incarnation == auth.incarnation, "upload claim mismatch");
            }
        }
        for (digest, sub) in &broker.submissions {
            ensure!(valid_digest(digest) && sub.digest == *digest && sub.size > 0
                && sub.size <= MAX_SUBMISSION_BYTES as u64, "invalid submission binding");
            let auth = broker.authorizations.get(&sub.run_id).context("missing upload authorization")?;
            ensure!(auth.submission_digest.as_ref() == Some(digest)
                && auth.incarnation == sub.incarnation, "submission/authorization mismatch");
        }
        let mut tokens = std::collections::BTreeSet::new();
        let mut ids = std::collections::BTreeSet::new();
        for (digest, claim) in &ledger.claims {
            let sub = broker.submissions.get(digest).context("grading without a broker claim")?;
            let job = &claim.job;
            // Validate historical shape at its issue time without reviving authority.
            job.validate(job.issued_at)?;
            ensure!(job.submission_digest == *digest && job.run_id == sub.run_id
                && job.incarnation == sub.incarnation && valid_id(&claim.campaign)
                && ledger.scorer_keys.contains_key(&claim.held_out_key_version)
                && job.fencing_token <= ledger.high_water
                && tokens.insert(job.fencing_token) && ids.insert(&job.job_id), "invalid grading claim");
    ...
    fn save_broker(&self, next: &BrokerLedger) -> anyhow::Result<()> {
        ...
        if result.is_err() { self.broker_poisoned.store(true, Ordering::SeqCst); }
        result
    }

    fn save_ledger(&self, next: &GradingLedger) -> anyhow::Result<()> {
        let result = persist_private(&self.dir.join("ledger.json"), next);
        if result.is_err() { self.grading_poisoned.store(true, Ordering::SeqCst); }
        result
    }
    ...
    fn request_slot(&self) -> bool {
        ...
        if rate.start.elapsed() >= Duration::from_secs(60) {
            *rate = RateWindow { start: Instant::now(), count: 0 };
        }
        if rate.count >= MAX_UPLOADS_PER_MINUTE { return false; }
        rate.count += 1;
        true
    }

    fn issue_upload(&self, run: &RunRecord, now: u64) -> anyhow::Result<UploadGrant> {
        ensure!(!self.broker_poisoned.load(Ordering::SeqCst), "broker unavailable");
        ensure!(run.state == RunState::Active && run.epoch >= 1, "upload requires an active eval");
        let incarnation = run.incarnation.as_ref().context("run incarnation absent")?;
        ensure!(valid_id(&run.run_id) && valid_id(incarnation), "run identity invalid");
        let mut broker = self.broker.lock().unwrap();
        ensure!(now >= broker.wall_high_water, "trusted clock rollback");
        ensure!(!broker.authorizations.contains_key(&run.run_id)
            && broker.authorizations.len() < MAX_UPLOAD_AUTHORIZATIONS, "upload already authorized or quota reached");
        let token = random_hex(32);
        let expires_at = now.checked_add(UPLOAD_TTL_S).context("upload expiry overflow")?;
        let mut next = broker.clone();
        next.wall_high_water = now;
        next.authorizations.insert(run.run_id.clone(), UploadAuthorization {
            incarnation: incarnation.clone(), token_digest: sha256_hex(token.as_bytes()),
            expires_at, attempts: 0, submission_digest: None,
        });
        self.save_broker(&next)?;
        *broker = next;
        Ok(UploadGrant { run_id: run.run_id.clone(), upload_token: token,
            expires_at, max_submission_bytes: MAX_SUBMISSION_BYTES })
    }
    ...
    fn reserve_upload(&self, run: &RunRecord, token: &str, now: u64) -> anyhow::Result<()> {
        ...
        ensure!(run.state == RunState::Active && run.epoch >= 1, "late submission");
        ensure!(token.len() == 64 && valid_digest(token), "upload token shape");
        let mut broker = self.broker.lock().unwrap();
        ensure!(now >= broker.wall_high_water, "trusted clock rollback");
        let auth = broker.authorizations.get(&run.run_id).context("no upload authorization")?;
        ensure!(auth.token_digest == sha256_hex(token.as_bytes()) && now < auth.expires_at
            && run.incarnation.as_ref() == Some(&auth.incarnation), "upload authorization mismatch");
        ensure!(auth.attempts < MAX_UPLOAD_ATTEMPTS && auth.submission_digest.is_none(), "upload consumed");
        let mut next = broker.clone();
        if next.minute != now / 60 { next.minute = now / 60; next.minute_attempts = 0; }
        ensure!(next.minute_attempts < MAX_UPLOADS_PER_MINUTE, "upload rate limit");
    ...
    fn store_upload(&self, run: &RunRecord, token: &str, bytes: &[u8], now: u64) -> anyhow::Result<()> {
        ensure!(!self.broker_poisoned.load(Ordering::SeqCst), "broker unavailable");
        ensure!(run.state == RunState::Active && run.epoch >= 1, "eval ended during upload");
        ...
        let auth = broker.authorizations.get(&run.run_id).context("no upload authorization")?;
            ...
            && run.incarnation.as_ref() == Some(&auth.incarnation) && auth.attempts > 0
            && auth.submission_digest.is_none(), "upload consumed/mismatched");
        ensure!(broker.submissions.len() < MAX_SUBMISSIONS && !broker.submissions.contains_key(&digest),
            "submission duplicate or intake quota");
        let object = self.dir.join("objects").join(&digest);
        match OpenOptions::new().write(true).create_new(true).mode(0o400).open(&object) {
            Ok(mut file) => {
                file.write_all(bytes)?;
                file.sync_all()?;
        ...
        let mut next = broker.clone();
        next.wall_high_water = now;
        next.authorizations.get_mut(&run.run_id).unwrap().submission_digest = Some(digest.clone());
        next.submissions.insert(digest.clone(), Submission { run_id: run.run_id.clone(),
            incarnation: run.incarnation.clone().context("incarnation")?, digest,
            size: bytes.len() as u64, stored_at: now });
    ...
    fn read_object(&self, digest: &str, size: u64) -> anyhow::Result<Vec<u8>> {
        ensure!(valid_digest(digest) && size > 0 && size <= MAX_SUBMISSION_BYTES as u64, "CAS binding");
        let path = self.dir.join("objects").join(digest);
        protected(&path, false)?;
        let file = OpenOptions::new().read(true).custom_flags(libc::O_NOFOLLOW).open(path)?;
        ensure!(file.metadata()?.len() == size, "CAS size mismatch");
        let mut bytes = Vec::new();
        file.take(MAX_SUBMISSION_BYTES as u64 + 1).read_to_end(&mut bytes)?;
        ensure!(bytes.len() as u64 == size && sha256_hex(&bytes) == digest, "CAS digest mismatch");
        Ok(bytes)
    }
    ...
    fn claim(&self, run: &RunRecord, request: &AuthorizeBody, now: u64) -> anyhow::Result<Dispatch> {
        ensure!(!self.grading_poisoned.load(Ordering::SeqCst), "grading unavailable");
        ensure!(run.state.terminal() && run.epoch >= 1 && run.termination_confirmed_at.is_some(),
            "grading requires a terminal eval and trusted teardown confirmation");
        ensure!(valid_id(&request.campaign) && valid_id(&request.held_out_key_version), "operator labels invalid");
        let submission = {
            let broker = self.broker.lock().unwrap();
            let auth = broker.authorizations.get(&run.run_id).context("no submission authorization")?;
            ensure!(run.incarnation.as_ref() == Some(&auth.incarnation), "run incarnation mismatch");
            let digest = auth.submission_digest.as_ref().context("no captured submission")?;
            broker.submissions.get(digest).context("missing submission claim")?.clone()
        };
        ...
        ensure!(ledger.scorer_keys.contains_key(&request.held_out_key_version), "unknown pinned key version");
        ensure!(ledger.spent < ledger.max_jobs, "cumulative grading budget exhausted");
        ensure!(!ledger.claims.contains_key(&submission.digest), "submission already consumed; no relaunch");
        ...
        let token = next.high_water.checked_add(1).context("grading fencing exhausted")?;
        let job = GradingJob {
            v: PROTO_V, kind: JOB_TYPE.into(), aud: AUD_GRADER.into(),
            job_id: ulid::Ulid::new().to_string(), run_id: run.run_id.clone(),
            incarnation: submission.incarnation, fencing_token: token, issued_at: now,
            expires_at: now.checked_add(MAX_GRADING_TTL_S).context("grading expiry overflow")?,
            submission_digest: submission.digest.clone(), task_id: TASK_ID.into(),
        ...
        next.spent += 1;
        next.wall_high_water = now;
        next.claims.insert(submission.digest, Claim { job: job.clone(),
            campaign: request.campaign.clone(), held_out_key_version: request.held_out_key_version.clone(),
    ...
    fn abandon(&self, job_id: &str) -> anyhow::Result<()> {
        ...
        let mut ledger = self.ledger.lock().unwrap();
        let mut next = ledger.clone();
        let claim = next.claims.values_mut().find(|claim| claim.job.job_id == job_id).context("unknown dispatch")?;
        if claim.state == JobState::DispatchConsumed {
            claim.state = JobState::Abandoned;
    ...
    fn commit_result(&self, run: &RunRecord, signed: &Signed, now: u64) -> anyhow::Result<()> {
        ...
        ensure!(run.state.terminal() && run.termination_confirmed_at.is_some(), "eval stop authority lost");
        // Decode only a bounded lookup hint; no hint grants authority or selects a filesystem path.
        ensure!(serde_json::to_vec(signed)?.len() <= MAX_GRADING_ENVELOPE_BYTES, "result envelope size");
        let hint: GradingResult = serde_json::from_str(&signed.payload)?;
        let mut ledger = self.ledger.lock().unwrap();
        ensure!(now >= ledger.wall_high_water, "trusted clock rollback");
        let claim = ledger.claims.get(&hint.submission_digest).context("unknown grading claim")?;
        ensure!(claim.job.run_id == run.run_id && run.incarnation.as_ref() == Some(&claim.job.incarnation),
            "run identity changed");
        let key = pubkey_from_hex(&ledger.scorer_keys[&claim.held_out_key_version])?;
        verify_result(signed, &key, &claim.job, now)?;
        if claim.state == JobState::ResultCommitted {
            ensure!(claim.result.as_ref() == Some(signed), "conflicting replacement result");
...
pub(crate) fn configure(
    ...
    if !args.grading_enable {
        ensure!(!args.grading_init && args.grading_budget.is_none(), "grading init requires --grading-enable");
        return Ok(None);
    }
    ensure!(operator_listen.is_some_and(|addr| addr.ip().is_loopback()),
        "grading requires a separate loopback operator API on :7101");
    for (addr, port) in [(args.grading_upload_listen, 7202), (args.grading_results_listen, 7201)] {
        ensure!(addr.ip().is_loopback() || (addr.ip().to_string() == "10.20.0.1" && addr.port() == port),
            "grading listener must use its designated controller WG tuple (or local test loopback)");
    }
    let key_file = args.grading_scorer_keys_file.as_ref().context("scorer key registry required")?;
    let meta = std::fs::symlink_metadata(key_file)?;
    ensure!(meta.is_file() && !meta.file_type().is_symlink() && meta.len() <= 16384
        && meta.uid() == unsafe { libc::geteuid() } && meta.permissions().mode() & 0o022 == 0,
    ...
    for key in keys.values() {
        ...
        ensure!(scorer != controller.verifying_key() && !hostd_keys.contains(&scorer),
            "scorer role must use a separate key");
    }
    Ok(Some(Arc::new(GradingService::open(root, args.grading_init, args.grading_budget, keys)?)))
}
...
async fn upload(State(app): S, AxPath(run_id): AxPath<String>, request: Request<Body>) -> impl IntoResponse {
    ...
    let Some(token) = request.headers().get("authorization").and_then(|value| value.to_str().ok())
        .and_then(|value| value.strip_prefix("Bearer ")).map(str::to_owned) else { return reject(); };
    {
        let ctl = app.ctl.lock().unwrap();
        let Some(run) = ctl.runs.get(&run_id) else { return reject(); };
        if service.reserve_upload(run, &token, now_unix()).is_err() { return reject(); }
    }
    let bytes = match tokio::time::timeout(Duration::from_secs(5), to_bytes(request.into_body(), MAX_SUBMISSION_BYTES)).await {
        Ok(Ok(bytes)) => bytes,
        _ => return reject(),
    };
    let ctl = app.ctl.lock().unwrap();
...
pub(crate) fn upload_router() -> Router<Arc<App>> {
    Router::new().route("/submissions/{run_id}", post(upload))
        .fallback(upload_fallback).method_not_allowed_fallback(upload_fallback)
}
...
struct Dispatch { job: GradingJob, bytes: Vec<u8> }

#[derive(Serialize)]
#[serde(rename_all = "snake_case")]
enum DispatchStatus { DispatchConsumed, DispatchUncertain }

#[derive(Serialize)]
struct DispatchAck { job_id: String, status: DispatchStatus }

fn service(app: &App) -> Result<&Arc<GradingService>, (StatusCode, String)> {
    app.grading.as_ref().ok_or_else(|| bad(StatusCode::NOT_FOUND, "grading disabled"))
}

async fn issue_upload(State(app): S, headers: HeaderMap, AxPath(id): AxPath<String>) -> super::Resp<UploadGrant> {
    check_operator(&app, &headers)?;
    let service = service(&app)?;
    let ctl = app.ctl.lock().unwrap();
    let run = ctl.runs.get(&id).ok_or_else(|| bad(StatusCode::NOT_FOUND, "no such run"))?;
    service.issue_upload(run, now_unix()).map(Json)
        .map_err(|error| bad(StatusCode::CONFLICT, error.to_string()))
}

async fn authorize(State(app): S, headers: HeaderMap, AxPath(id): AxPath<String>, Json(body): Json<AuthorizeBody>) -> super::Resp<DispatchAck> {
    check_operator(&app, &headers)?;
    let service = service(&app)?.clone();
    let run = app.ctl.lock().unwrap().runs.get(&id).cloned()
        .ok_or_else(|| bad(StatusCode::NOT_FOUND, "no such run"))?;
    let dispatch = service.claim(&run, &body, now_unix())
        .map_err(|error| bad(StatusCode::CONFLICT, error.to_string()))?;
    let signed = Signed::sign(&app.key, "controller", &dispatch.job);
    let frame = match encode_dispatch(&signed, &dispatch.bytes) {
        Ok(frame) => frame,
        Err(error) => {
            service.abandon(&dispatch.job.job_id).map_err(|e| bad(StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    ...
    let delivered = service.client.post(GRADER_DISPATCH_URL).header("content-type", "application/octet-stream")
        .body(frame).send().await.map(|response| response.status().is_success()).unwrap_or(false);
    let status = if delivered { DispatchStatus::DispatchConsumed } else {
    ...
    Ok(Json(DispatchAck { job_id: dispatch.job.job_id, status }))
}
...
async fn operator_status(State(app): S, headers: HeaderMap) -> super::Resp<OperatorStatus> {
    ...
    Ok(Json(OperatorStatus { policy: "zero_agent_feedback", lifetime_budget: service.anchor.max_jobs,
        spent: ledger.spent, high_water: ledger.high_water, jobs: ledger.claims.values().cloned().collect() }))
}
...
async fn operator_results(State(app): S, headers: HeaderMap) -> super::Resp<Vec<Signed>> {
    ...
    let service = service(&app)?;
    let ledger = service.ledger.lock().unwrap();
    Ok(Json(ledger.claims.values().filter_map(|claim| claim.result.clone()).collect()))
}

pub(crate) fn operator_router() -> Router<Arc<App>> {
    Router::new().route("/grading/runs/{id}/upload-authorization", post(issue_upload))
...
#[derive(Serialize)]
#[serde(rename_all = "snake_case")]
enum AppendStatus { Appended, Rejected }
#[derive(Serialize)]
struct AppendAck { status: AppendStatus }

async fn append_result(State(app): S, request: Request<Body>) -> impl IntoResponse {
    let reject = || (StatusCode::BAD_REQUEST, Json(AppendAck { status: AppendStatus::Rejected }));
    let Some(service) = app.grading.as_ref() else { return reject(); };
    let bytes = match tokio::time::timeout(Duration::from_secs(5), to_bytes(request.into_body(), MAX_GRADING_ENVELOPE_BYTES)).await {
        Ok(Ok(bytes)) => bytes,
        _ => return reject(),
    };
    #[derive(Deserialize)]
    #[serde(deny_unknown_fields)]
    struct Envelope { payload: String, sig_hex: String, signer: String }
    let raw: Envelope = match serde_json::from_slice(&bytes) { Ok(value) => value, Err(_) => return reject() };
    let signed = Signed { payload: raw.payload, sig_hex: raw.sig_hex, signer: raw.signer };
    let hint: GradingResult = match serde_json::from_str(&signed.payload) { Ok(value) => value, Err(_) => return reject() };
    let run = match app.ctl.lock().unwrap().runs.get(&hint.run_id).cloned() { Some(run) => run, None => return reject() };
    match service.commit_result(&run, &signed, now_unix()) {
        Ok(()) => (StatusCode::OK, Json(AppendAck { status: AppendStatus::Appended })),
        Err(error) => { warn!(error = %error, "grading append rejected"); reject() }
    }
}

async fn append_fallback() -> impl IntoResponse {
    (StatusCode::METHOD_NOT_ALLOWED, Json(AppendAck { status: AppendStatus::Rejected }))
}

pub(crate) fn results_router() -> Router<Arc<App>> {
    Router::new().route("/results", post(append_result))
        .fallback(append_fallback).method_not_allowed_fallback(append_fallback)
}

pub(crate) async fn start_listeners(args: &GradingArgs, app: Arc<App>) -> anyhow::Result<()> {
    if app.grading.is_none() { return Ok(()); }
