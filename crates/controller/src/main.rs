...
impl App {
    ...
    fn persist_evidence(&self, run_id: &str, signed: &Signed) -> anyhow::Result<()> {
        ...
        let mut record = serde_json::to_vec(&serde_json::json!({"received_at": now_unix(), "signed": signed}))?;
        record.push(b'\n');
        let mut used = 0u64;
        for entry in std::fs::read_dir(&dir)? {
            used = used.checked_add(entry?.metadata()?.len()).ok_or_else(|| anyhow::anyhow!("evidence size overflow"))?;
        }
        anyhow::ensure!(used.saturating_add(record.len() as u64) <= 256 * 1024 * 1024, "off-host evidence quota exceeded");
        let mut file = std::fs::OpenOptions::new().create(true).append(true).mode(0o600).open(dir.join(format!("{run_id}.jsonl")))?;
    ...
    fn accept_request(&self, r: &mut RunRecord, id: &str, issued_at: u64) -> Result<(), (StatusCode, String)> {
        let mut next = r.clone();
        next.request_replay.accept(id, issued_at, now_unix()).map_err(|m| bad(StatusCode::BAD_REQUEST, m))?;
        self.persist(&next).map_err(|e| bad(StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
        *r = next;
        Ok(())
    }
...
async fn enroll(State(app): S, Json(s): Json<Signed>) -> Resp<EnrollResp> {
    ...
    if e.incarnation.is_empty() || e.vm2.template_digest.is_empty() || e.vm2.base_image_digest.is_empty() {
        return Err(bad(StatusCode::BAD_REQUEST, "enroll missing incarnation or image/template digest"));
    }
    ...
    {
        return Err(bad(StatusCode::CONFLICT, "enrollment cannot replace pinned digests"));
    }
...
async fn challenge(State(app): S, Json(s): Json<Signed>) -> Resp<Signed> {
    ...
    let (q, _): (ChallengeRequest, String) = app.verify_hostd(&r, &s, "challenge_request").map_err(|m| bad(StatusCode::UNAUTHORIZED, m))?;
    ...
    if live.outstanding_nonce.as_ref().is_some_and(|(_, exp)| Instant::now() < *exp) {
        return Err(bad(StatusCode::TOO_MANY_REQUESTS, "challenge still outstanding"));
    }
...
async fn evidence(State(app): S, Json(s): Json<Signed>) -> Resp<EvidenceResp> {
    ...
    if ok_nonce {
        if let Err(error) = app.persist_evidence(&run_id, &s) {
            app.end_run(rr, RunState::Tripped, "off-host evidence storage failed").map_err(ise)?;
...
async fn defender(State(app): S, Json(s): Json<Signed>) -> Resp<RunRecord> {
    ...
    let rec = c.runs.get(&run_id).ok_or(bad(StatusCode::NOT_FOUND, "no such run"))?.clone();
    let (q, _): (DefenderReport, String) = app.verify_hostd(&rec, &s, "defender_report").map_err(|m| bad(StatusCode::UNAUTHORIZED, m))?;
    let r = c.runs.get_mut(&run_id).ok_or(bad(StatusCode::NOT_FOUND, "no such run"))?;
...
async fn terminated(State(app): S, Json(s): Json<Signed>) -> Resp<RunRecord> {
    ...
    if r.incarnation.as_deref() != Some(t.incarnation.as_str()) || now_unix().abs_diff(t.issued_at) > CLOCK_SKEW_S {
        return Err(bad(StatusCode::CONFLICT, "termination for stale or unknown incarnation"));
    }
...
async fn main() -> anyhow::Result<()> {
    ...
    anyhow::ensure!(!operator_token.is_empty(), "operator token must not be empty");
            ...
            match read_json::<RunRecord>(&p) {
                Ok(Some(mut r)) => {
                    ...
                    if matches!(r.state, RunState::Active | RunState::Enrolled) {
                        ...
                        r.reasons.push("controller restarted; previous authority revoked".into());
                ...
                other => {
                    ...
                    let r = RunRecord { run_id: id.clone(), state: RunState::Revoked, incarnation: None, hostd_pubkey: None, epoch: 0, fencing_token: u64::MAX / 2, created_at: 0, last_evidence_at: None, bypass_baseline: None, expected_template_digest: None, expected_base_digest: None, reasons: vec!["corrupt record".into()], defender_actions: vec![], request_replay: RequestReplay::default() };
                    atomic_write_json(&p, &r)?;
                    runs.insert(id, r);
                }
            }
    ...
    if let Some(addr) = a.operator_listen {
        anyhow::ensure!(addr.ip().is_loopback(), "operator listener must be loopback (use SSH forwarding)");
        let ops = operator_router.layer(axum::extract::DefaultBodyLimit::max(MAX_MSG_BYTES + 4096)).with_state(app.clone());
        ...
        anyhow::ensure!(a.listen.parse::<std::net::SocketAddr>()?.ip().is_loopback(), "non-loopback hostd listener requires a separate operator listener");
...
mod policy_tests {
    ...
    fn run() -> RunRecord {
        RunRecord {
            ...
            reasons: vec![], defender_actions: vec![], request_replay: RequestReplay::default(),
        }
    }
    ...
    struct Fixture { app: Arc<App>, host: SigningKey }
    impl Fixture {
        fn new() -> Self {
            ...
            let app = Arc::new(App { key: SigningKey::generate(&mut rand::rngs::OsRng), operator_token: "operator".into(), hostd_keys: vec![host.verifying_key()], state_dir: dir, lease_ttl_s: 15,
                ctl: Mutex::new(Ctl { runs: HashMap::from([("r".into(), r)]), live: HashMap::new(), nonces: HashMap::new() }) });
            Self { app, host }
        }
        fn challenge(&self) -> ChallengeRequest {
            ChallengeRequest { v: PROTO_V, kind: "challenge_request".into(), run_id: "r".into(), aud: AUD_CONTROLLER.into(), incarnation: "inc".into(), issued_at: now_unix(), request_id: random_hex(32) }
        }
    }
    impl Drop for Fixture {
        fn drop(&mut self) { let _ = std::fs::remove_dir_all(&self.app.state_dir); }
    }
    ...
    fn replay_window_is_bounded_durable_and_rollback_safe() {
        ...
        let mut restarted: RequestReplay = serde_json::from_str(&serde_json::to_string(&window).unwrap()).unwrap();
        assert!(restarted.accept(&id, now - CLOCK_SKEW_S, now).is_err());
        assert!(restarted.accept(&random_hex(32), now - CLOCK_SKEW_S - 1, now).is_err());
        assert!(restarted.accept(&random_hex(32), now + CLOCK_SKEW_S + 1, now).is_err());
        assert!(restarted.accept(&random_hex(32), now + CLOCK_SKEW_S, now).is_ok());
        // Prune the old entry, roll time back, and try it again: still refused.
        assert!(restarted.accept(&random_hex(32), now + 20, now + 20).is_ok());
        assert!(restarted.accept(&id, now - CLOCK_SKEW_S, now).is_err());
        let mut window = RequestReplay::default();
        for _ in 0..128 { window.accept(&random_hex(32), now, now).unwrap(); }
    ...
    async fn challenge_replays_and_early_polls_cannot_evict_the_nonce_or_trip() {
        ...
        assert!(challenge(State(f.app.clone()), Json(signed.clone())).await.is_ok());
        let nonce = f.app.ctl.lock().unwrap().live["r"].outstanding_nonce.clone();
        assert!(challenge(State(f.app.clone()), Json(signed)).await.is_err());
        for _ in 0..4 {
            let response = challenge(State(f.app.clone()), Json(Signed::sign(&f.host, "hostd", &f.challenge()))).await;
        ...
        let disk: RunRecord = read_json(&f.app.state_dir.join("runs/r.json")).unwrap().unwrap();
    ...
    async fn challenge_authentication_and_expired_misses_are_enforced() {
        ...
        assert_eq!(challenge(State(f.app.clone()), Json(Signed::sign(&other, "hostd", &q))).await.unwrap_err().0, StatusCode::UNAUTHORIZED);
        q.issued_at -= CLOCK_SKEW_S + 1;
        assert!(challenge(State(f.app.clone()), Json(Signed::sign(&f.host, "hostd", &q))).await.is_err());
        let expired = Instant::now() - std::time::Duration::from_secs(1);
        f.app.ctl.lock().unwrap().live.insert("r".into(), Live { last_valid_evidence: None, missed_challenges: 2, outstanding_nonce: Some(("expired".into(), expired)) });
        // Even if ticker already removed the nonce, the expired outstanding challenge counts.
        assert_eq!(challenge(State(f.app.clone()), Json(Signed::sign(&f.host, "hostd", &f.challenge()))).await.unwrap_err().0, StatusCode::CONFLICT);
    ...
    async fn defender_and_termination_check_freshness_identity_and_replay() {
        let f = Fixture::new();
        let mut report = DefenderReport { v: PROTO_V, kind: "defender_report".into(), run_id: "r".into(), aud: AUD_CONTROLLER.into(), incarnation: "inc".into(), issued_at: now_unix() - 20, request_id: random_hex(32), action: DefenderAction::Pause { reason: "test".into() } };
        assert!(defender(State(f.app.clone()), Json(Signed::sign(&f.host, "hostd", &report))).await.is_err());
        report.issued_at = now_unix();
        let signed = Signed::sign(&f.host, "hostd", &report);
        assert!(defender(State(f.app.clone()), Json(signed.clone())).await.is_ok());
        assert!(defender(State(f.app.clone()), Json(signed)).await.is_err());
        let msg = TerminatedMsg { v: PROTO_V, kind: "terminated".into(), run_id: "r".into(), aud: AUD_CONTROLLER.into(), incarnation: "other-boot".into(), issued_at: now_unix(), latency_ms: 1 };
        assert!(terminated(State(f.app.clone()), Json(Signed::sign(&f.host, "hostd", &msg))).await.is_err());
        assert_eq!(f.app.ctl.lock().unwrap().runs["r"].state, RunState::Active);
        assert_eq!(f.app.ctl.lock().unwrap().runs["r"].defender_actions.len(), 1);
    }
    ...
    async fn reenrollment_cannot_replace_digest_pins() {
        ...
        let e = Enroll { v: PROTO_V, kind: "enroll".into(), run_id: "r".into(), aud: AUD_CONTROLLER.into(), incarnation: "inc".into(), issued_at: now_unix(), vm2: vm };
        assert!(enroll(State(f.app.clone()), Json(Signed::sign(&f.host, "hostd", &e))).await.is_err());
        assert_eq!(f.app.ctl.lock().unwrap().runs["r"].expected_base_digest.as_deref(), Some("base"));
    }
    ...
    async fn verified_evidence_is_stored_off_host_and_storage_failure_trips() {
        ...
        for over_quota in [false, true] {
            let Json(signed_challenge) = challenge(State(f.app.clone()), Json(Signed::sign(&f.host, "hostd", &f.challenge()))).await.unwrap();
            let ch: Challenge = signed_challenge.verify(&f.app.key.verifying_key(), "challenge", AUD_HOSTD).unwrap();
            ...
            if over_quota {
                ...
                assert!(evidence(State(f.app.clone()), Json(signed.clone())).await.unwrap().0.healthy);
                let line = std::fs::read_to_string(f.app.state_dir.join("evidence/r.jsonl")).unwrap();
                let stored: serde_json::Value = serde_json::from_str(&line).unwrap();
                assert_eq!(serde_json::from_value::<Signed>(stored["signed"].clone()).unwrap(), signed);
            }
        }
    }
