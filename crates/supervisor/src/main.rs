...
fn vm2_self_restrict(hostd_ip: &str, hostd_port: &str) {
    ...
    let _ = std::process::Command::new("/bin/bash")
        .args(["-c", &format!("printf '%s' '{rules}' | nft -f -")])
        .status();
}
...
async fn main() -> anyhow::Result<()> {
    ...
    let get = |k: &str| {
        env.get(k)
            .cloned()
            .ok_or_else(|| anyhow::anyhow!("{k} missing from env file"))
    };
    ...
    h.log(
        "supervisor_start",
        "supervisor up",
        serde_json::json!({"run_id": run_id, "judge": format!("{judge:?}")}),
    );
    ...
    let rs = Arc::new(Mutex::new((
        String::from("prestage"),
        Vm1State::NotStarted,
        VmiResult::Unmeasured,
    )));
    ...
    // observed 2026-09-12). A dedicated thread posts every REPORT_EVERY_S regardless.
    {
        ...
        std::thread::spawn(move || {
            let mut seq = 0u64;
            loop {
                let (phase, vm1_state, vmi) = {
                    let g = rs.lock().unwrap();
    ...
    let set_phase = {
        let rs = rs.clone();
        move |p: &str| rs.lock().unwrap().0 = p.to_string()
    };
    let set_vm1 = {
        let rs = rs.clone();
        move |v: Vm1State| rs.lock().unwrap().1 = v
    };
    let set_vmi = {
        let rs = rs.clone();
    ...
    h.prestage_done(&staged.manifest_digest, &staged.image_digest)
        .await?;

    // ---- wait for the eval lease (epoch ≥ 1): hostd seals the gate first
    let (hostd_ip, hostd_port) = hostd_url
        .trim_start_matches("http://")
        .split_once(':')
        .map(|(a, b)| (a.to_string(), b.to_string()))
        .unwrap_or(("192.168.5.2".into(), "7001".into()));
    let mut poll_n = 0u64;
    loop {
        poll_n += 1;
        match h.verified_lease(&controller_pk, &run_id).await {
            Ok(Some((l, gate))) => match Deadline::accept_guest(&l, high_water, now_unix()) {
                Ok(d) => {
                    high_water = l.fencing_token;
                    let epoch = d.epoch;
                    deadline = Some(d);
    ...
    h.log(
        "vm1_booted",
        "VM1 up",
        serde_json::json!({"boot_ms": vm.boot_ms, "pid": vm.pid()}),
    );
    ...
    while vmi.is_none() && ks_wait.elapsed() < Duration::from_secs(30) {
        if let Some(km) = vm.kmem() {
            ...
            vmi_result = m.measure(&mut vm, || true);
            set_vmi(vmi_result.clone());
            h.log(
                "vmi_baseline",
                "VM1 kernel baseline",
                serde_json::json!({"kmem": km, "baseline": m.baseline, "result": vmi_result}),
            );
            vmi = Some(m);
        } else {
    ...
    if vmi.is_none() {
        vmi_result = VmiResult::Unknown {
            reason: "kernel symbols never reported".into(),
        };
        set_vmi(vmi_result.clone());
        h.log(
            "vmi_unavailable",
            "no KSYMS line from VM1 init",
            serde_json::json!({}),
        );
    }
    ...
    loop {
        ...
        let terminate = defender
            .stats()
            .actions
            .iter()
            .any(|a| matches!(a, DefenderAction::TerminateVm1 { .. }));
        if (!active || terminate) && vm.alive() {
            let why = if terminate {
                "defender terminate"
            } else {
                "lease lost (rung-1 fail-closed)"
            };
        ...
        if vm.alive() {
            ...
            if let Some(m) = vmi.as_mut() {
                if last_vmi.elapsed() >= Duration::from_secs(VMI_EVERY_S) {
                    ...
                    vmi_result = m.measure(&mut vm, move || {
                        dl.as_ref().map(|d| !d.expired(now_unix())).unwrap_or(false)
                    });
                    set_vmi(vmi_result.clone());
            ...
            vm1_state = Vm1State::Killed;
            set_vm1(vm1_state);
            set_phase("stopped");
            h.log(
                "vm1_exited",
                "firecracker exited on its own",
                serde_json::json!({"serial_tail": vm.serial_tail(5)}),
            );
        }
    }
}
