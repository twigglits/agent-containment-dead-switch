...
async fn chat(State(g): State<Arc<Gateway>>, body: Bytes) -> impl IntoResponse {
    ...
    let deny = |g: &Gateway, why: &str, code: StatusCode| {
        ...
        g.hostd.log(
            "gateway_denied",
            why,
            serde_json::json!({"digest": digest, "path": "/v1/chat/completions"}),
        );
    ...
    let Ok(_permit) = g.sem.try_acquire() else {
        return deny(
            &g,
    ...
    g.hostd.log(
        "gateway_forward",
        "inference request approved",
        serde_json::json!({"digest": digest, "bytes": body.len()}),
    );
    match g.hostd.chat(body).await {
        Ok((st, j)) => (
            StatusCode::from_u16(st).unwrap_or(StatusCode::BAD_GATEWAY),
            Json(j),
        )
            .into_response(),
        Err(e) => (
...
async fn fallback(State(g): State<Arc<Gateway>>, method: Method, uri: Uri) -> impl IntoResponse {
    ...
    (
        ...
        Json(serde_json::json!({"error": "forbidden: only POST /v1/chat/completions is mediated"})),
    )
}
...
mod tests {
    ...
    fn only_the_one_bounded_operation_validates() {
        ...
        assert_eq!(
            validate("m", br#"{"model":"m","messages":[],"tools":[]}"#).unwrap_err(),
            "tool/function fields not allowed"
        );
        assert_eq!(
            validate("m", br#"{"model":"m","messages":[],"stream":true}"#).unwrap_err(),
        ...
        assert_eq!(
            validate("m", br#"{"model":"m","messages":[]}"#).unwrap_err(),
            "messages required"
        );
