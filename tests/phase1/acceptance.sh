...
inst_gone(){
  local name="vm2-$(echo "$1"|tr A-Z a-z)" out
...
gw=$(sudo -n -u _deadswitch /bin/bash -c 'curl -s -m4 -o /dev/null -w "%{http_code}" http://192.168.5.2:7001/ 2>/dev/null' 2>/dev/null || echo x)
...
harness_seen=no
for i in $(seq 1 420); do   # up to ~14 min: prestage (uv sync) + VM1 boot + agentic inference
