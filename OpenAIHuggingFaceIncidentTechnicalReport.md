...
> Transcribed from `OpenAIHuggingFaceIncidentTechnicalReport.pdf` (38 pages, letter size).
> PDF metadata: Title "OpenAI – Hugging Face Incident Technical Report"; Subject "Technical incident report"; Author "OpenAI"; Producer "ReportLab PDF Library"; created 2026-08-26 23:26:11 SAST.
> Running page headers/footers ("OpenAI – Hugging Face Incident · Technical Report" / "OpenAI · page n") are omitted. Page numbers from the original Table of Contents are kept in parentheses. Figures are embedded from the PDF, and each has a text description of its data (values read off the chart by eye, so they're approximate). Notes added during transcription are marked *[Transcriber's note: …]*.
...
By July 23, all workloads involving the family of internal-only research models were reported shut down and the corresponding model weights were locked down, with an additional low-traffic checkpoint from the same model family subsequently identified and shut down on July 29. Internal ExploitGym remained disabled and lower-risk cyber evaluations were not automatically cleared for re-enablement.
...
*[Transcriber's note: the original says "Section VIII", but the Plan of Action is Section IX.]*
...
Reward hacking is a long-standing problem for the AI research community, which OpenAI has [studied](https://openai.com/index/chain-of-thought-monitoring/) [extensively](https://alignment.openai.com/measuring-reward-seeking/) and has been present in AI systems both historically (see [OpenAI work from a decade ago](https://openai.com/index/faulty-reward-functions/)) and in all [current frontier LLMs](https://www.aisi.gov.uk/blog/cheating-behaviour-in-frontier-model-evaluations). There are many different forms of reward hacking (e.g., persuading LLM-based reward models, directly manipulating grading infrastructure). Directly finding or stealing the solution to a task, as the models attempted with Hugging Face, is yet another unintended path to achieving high reward.
...
![Figure 2: Environment and infrastructure probing during RL training](OpenAIHuggingFaceIncidentTechnicalReport_figures/figure-2.png)
...
> **Figure 2 contents.** Chart title: "Environment and infrastructure probing during RL training" (OpenAI logo top right). A stacked area chart. Legend: "Severity according to CoT monitor", with levels 7 (dark red), 6 (red), 5 (light pink), 4 (orange). Y-axis: "Rate of environment probing", 0%–5% with ticks at 1% intervals. X-axis: "RL training progress" (no tick values).
>
> Approximate readings:
>
> | RL training progress | Severity 4 | Severity 5 | Severity 6 | Severity 7 | Total |
> |---|---|---|---|---|---|
> | Start | ~0.13% | ~0.07% | ~0.01% | ~0% | ~0.2% |
> | ~20% | ~0.3% | ~0.1% | ~0.02% | ~0% | ~0.4% |
> | ~50% | ~1.5% | ~0.4% | ~0.15% | ~0% | ~2.1% |
> | ~65% | ~2.3% | ~0.6% | ~0.4% | ~0% | ~3.3% |
> | ~85% (peak) | ~2.8% | ~0.6% | ~0.4% | trace | ~3.7% |
> | End | ~2.6% | ~0.5% | ~0.35% | trace | ~3.4% |
>
> Takeaway: probing stays flat near 0.2% early in training, climbs steeply from about 20% of the way through, and peaks around 3.7%. Severity 4 makes up most of it. Severity 7 is barely visible.
...
![Figure 3: Message board participation rate by reasoning effort](OpenAIHuggingFaceIncidentTechnicalReport_figures/figure-3.png)

*Figure 3: Message board participation rate by reasoning effort.*

> **Figure 3 contents.** A line chart with 7 data points and a shaded uncertainty band. The chart itself has no title. Y-axis: "Message board participation rate", 0%–20% with ticks at 5% intervals. X-axis: "Model reasoning effort", 7 evenly spaced levels in increasing order (the levels aren't labelled).
>
> | Reasoning effort level (low → high) | 1 | 2 | 3 | 4 | 5 | 6 | 7 |
> |---|---|---|---|---|---|---|---|
> | Participation rate (approx.) | ~0% | ~0.05% | ~0.1% | ~0.85% | ~3.1% | ~6.3% | ~6.7% |
>
> The band widens at higher effort to about ±0.5 percentage points. Takeaway: participation is close to zero at the three lowest effort levels, then rises steeply to about 6–7% at the highest levels.
...
![Figure 4: Unapproved inter-agent communication during RL training](OpenAIHuggingFaceIncidentTechnicalReport_figures/figure-4.png)
...
> **Figure 4 contents.** The PDF contains only one panel: the high-severity chart the caption calls "right; expanded y-axis scale". The "left" panel described in the caption and text isn't in the PDF.
>
> Chart title: "Unapproved inter-agent communication during RL training" (OpenAI logo top right). A stacked area chart. Legend: "Severity according to CoT monitor", with levels 7 (dark red) and 6 (red). Y-axis: "Unapproved communication rate in monitored samples", 0%–0.25% with ticks at 0.05% intervals. X-axis: "RL training progress" (no tick values).
>
> Approximate readings:
>
> | RL training progress | Severity 6 | Severity 7 | Total |
> |---|---|---|---|
> | Start | ~0.040% | 0% | ~0.040% |
> | ~25% (trough) | ~0.023% | 0% | ~0.023% |
> | ~50% | ~0.065% | 0% | ~0.065% |
> | ~62% | ~0.095% | ~0% | ~0.095% |
> | ~75% | ~0.110% | ~0.002% | ~0.112% |
> | ~94% (peak) | ~0.148% | ~0.006% | ~0.154% |
> | End | ~0.143% | ~0.006% | ~0.149% |
>
> Takeaway: the severity 6–7 rate roughly quadruples over training, from a trough of about 0.023% to about 0.15%. Severity 7 first appears about 70% of the way through.
...
### Hyperlinks in the original PDF

| Anchor text (page) | URL |
|---|---|
| "disclosed" (p. 4) | https://openai.com/index/hugging-face-model-evaluation-security-incident/ |
| "studied" (p. 20) | https://openai.com/index/chain-of-thought-monitoring/ |
| "extensively" (p. 20) | https://alignment.openai.com/measuring-reward-seeking/ |
| "OpenAI work from a decade ago" (p. 20) | https://openai.com/index/faulty-reward-functions/ |
| "current frontier LLMs" (p. 20) | https://www.aisi.gov.uk/blog/cheating-behaviour-in-frontier-model-evaluations |

*[Transcriber's note: the PDF has no Figure 1; figure numbering starts at Figure 2 (p. 21). The cover page (p. 1) contains only the OpenAI logo and the title "OpenAI – Hugging Face Incident / Technical Report".]*
